...
namespace {
  ...
  const double MaxRatio   = 7.04;  // When in trouble, we can step over reserved time with this ratio
  const double StealRatio = 0.35; // However we must not steal time from remaining moves over this ratio

  // Easy move, fail low + eval drop, no fail low, no eval drop, no fail low + no eval drop 
  const double searchFactor[5] = {0.119, 1.035, 0.761, 0.725, 0.361};
  ...
  double move_importance(int ply) {
    ...
    const double XScale = 7.66;
    const double XShift = 58.4;
    const double Skew   = 0.182;

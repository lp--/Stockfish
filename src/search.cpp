...
void Thread::search() {
         ...
         && !(Limits.depth && mainThread && rootDepth / ONE_PLY > Limits.depth))
  {
      ...
      if (mainThread)
          mainThread->bestMoveChanges *= 0.531, mainThread->failedLow = false;
      ...
      if (    Limits.use_time_management()
          ...
          && !Threads.stopOnPonderhit)
          {
              ...
              int improvingFactor = std::max(238, std::min(820, 276 + 108 * F[0] - 6 * F[1]));
              ...
              unstablePvFactor *= std::pow(mainThread->previousTimeReduction, 0.523) / timeReduction;

              // Stop the search if we have only one legal move, or if available time elapsed
              if (   rootMoves.size() == 1
                  || Time.elapsed() > Time.optimum() * unstablePvFactor * improvingFactor / 562)
              {
